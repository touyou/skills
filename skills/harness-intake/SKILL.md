...
| 状態 | 経路 |
|---|---|
| push 権限あり | 原則どおり Issue 起票。ユーザーが望めば直接 PR でもよいが、**PR か Issue かは必ずユーザーに確認**する（レビューを挟む運用が既定） |
...
（Issue 本文 + タイトルをそのまま格納）。権限が得られた後にこのスキルを再実行すると、
保留キューのファイルを検出して再送を提案し、送信できたらファイルを削除する。
